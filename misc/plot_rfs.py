"""look at how concordance changes with max depth/number of trees"""
import os
import pandas as pd
...
def plot_figure(df, int_cause, x_axis="max_depth", y_axis="mean_test_concordance"):
    ...
    plt.ylabel(y_axis)
    plt.xlabel(x_axis)
    plt.savefig(f"/home/j/temp/agesak/thesis/plots/{int_cause}_rf_trees.pdf")


for int_cause in ["x59", "y34"]:
    summaries = []
    for tree in ["50", "150", "100", "200", "400", "600", "800", "1000", "1200", "1600", "2000", "2200"]:
    ...
    summaries = pd.concat(summaries)
    summaries.columns = [x.replace("param_clf__estimator__", "") for x in list(summaries)]
